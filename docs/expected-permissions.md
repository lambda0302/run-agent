...
> 状态：**已随 0.8.0 落地（近期落地版），归档设计稿**。当前权限行为以 [permissions.md](permissions.md) 为准；
> 本文件保留设计决策过程与未落地的远期方向，供 V8 桶后续参考。
>
> 已落地（0.8.0，语义并入 permissions.md）：判定链**收口前置单线**（§1，V7-P1/P3 修复）、`run_bash`
> **六分类影响半径**（§2 近期落地版，R0 闭集自动 allow / dangerous 硬拒 / 其余 ask）、命令文本**三目录段危险段**
> （§2 五步判定第 1 步，V7-P4/P5）、acceptEdits 收紧只预授权 cwd 内文件写（§1，V7-P2）、判定矩阵测试（§10 铁律 4）。
>
> 未落地（远期，勿当现状）：完整五层智能分层 + R1 acceptEdits 放行 bash（§2，依赖沙箱）、记忆**来源分级**
> （§4 挂点 B，user 指令级 / agent 参考级）、MCP 从 readOnlyHint 免确认收紧为全 ask（§6）、沙箱引入（§11）。
>
> 文中「现状」均指 0.7.2 基线；修复后行号已漂移，本文件的语义判定优先于行号。
